...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('warn')
        .setDescription('Permet de Warn un membre sans signalement')
    ...
    async execute(interaction) {
        if (await roles.userIsMod(interaction.member)&&(await roles.userIsApprenti(interaction.member))==false) {
            ...
            const embed = createEmbeds.createFullEmbed('Vous avez été averti•e', 'Une de vos actions a été jugée comme inappropriée par ' + userMention(interaction.user.id) + ' pour la raison suivante : **' + reason + '**', null, null, 0x2f3136, null);
            await sanctionDB.saveSanction(userId.id_user, interaction.user.id, "warn", reason);

            const channel = await interaction.guild.channels.fetch(sanction);
            await channel.send({ content: '', embeds: [createEmbeds.createFullEmbed('warn', 'L\'utilisateur ' + userMention(userId.id_user) + ' a été warn par ' + userMention(interaction.user.id) + ' pour la raison suivante : **' + reason + '**', null, null, 0x2f3136, null)] });
            ...
            return await interaction.reply({ content: 'L\'utilisateur ' + userMention(userId.id_user) + 'à bien été warn', ephemeral: true });
